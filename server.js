...
app.get('/status', (q, r) => r.json({ streaming_api_configured: !!STREAM_KEY }));
...
app.get('/meta/movie/:id.json', async (req, res) => {
  try {
    ...
    if (req.query.debug) {
      ...
      return res.json({ meta, cache_hit: cacheHit, debug: sources, streaming_api_called: streamRaw !== null || dStream.value !== undefined, streaming_raw_it: streamRaw });
    }
    res.json({ meta });
  } catch (e) {
...
app.listen(PORT, () => console.log('Italy Release Checker 3.7 listening on ' + PORT));
