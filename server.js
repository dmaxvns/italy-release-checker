...
const releaseCache = new Map(); // tmdbId -> { releaseText, ts }
...
app.get('/meta/movie/:id.json', async (req, res) => {
  try {
    ...
    if (cached && (Date.now() - cached.ts) < CACHE_TTL_MS) {
      ...
      [dTmdb, dWiki] = await Promise.allSettled([tmdbTheatricalTvIT(tmdbId), wikidataIT(tmdbId)]);
...
app.listen(PORT, () => console.log('Italy Release Checker 3.4 listening on ' + PORT));
