  ...
  for (const [k, v] of Object.entries(obj)) {
    if (/audio/i.test(k)) {
      ...
      for (const item of arr) {
        if (typeof item === 'string' && /^it(-|$)/i.test(item)) return true;
        if (item && typeof item === 'object') {
          const lang = String(item.language || item.languageCode || item.locale || '').toLowerCase();
          if (/^it(-|$)/.test(lang)) return true;
        }
      }
    }
...
app.get('/meta/movie/:id.json', async (req, res) => {
  try {
    ...
    const sources = {
      ...
      tmdb_theatrical_tv: a, wikidata: b, streaming_audio_it: c
    };
...
app.listen(PORT, () => console.log('Italy Release Checker 3.0 listening on ' + PORT));
