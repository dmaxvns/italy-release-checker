...
// Fonte ufficiale via API TMDB (dati JustWatch senza scraping, niente blocchi anti-bot)
async function providersIT(id) {
  const d = await tmdb('/movie/' + id + '/watch/providers');
  const it = d?.results?.IT;
  if (!it) return false;
  return !!(it.flatrate?.length || it.rent?.length || it.buy?.length || it.free?.length || it.ads?.length);
}
...
app.get('/meta/movie/:id.json', async (req, res) => {
  try {
    ...
    const [a, b, c, d, e] = await Promise.allSettled([
      tmdbIT(tmdbId), imdbIT(m.imdb_id), wikidataIT(tmdbId), justwatch(m.title || m.original_title), providersIT(tmdbId)
    ]);
    const sources = { tmdb_release_dates: a, imdb_scrape: b, wikidata: c, justwatch_scrape: d, tmdb_watch_providers: e };
    ...
    // Apri /meta/movie/tmdb:ID.json?debug=1 nel browser per vedere quale fonte ha risposto true/false
    if (req.query.debug) {
      return res.json({
        meta,
...
app.listen(PORT, () => console.log('Italy Release Checker 1.8 listening on ' + PORT));
