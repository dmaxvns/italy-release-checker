...
const MANIFEST = {
  ...
  description: 'Controlla se un film risulta uscito in Italia.',
...
async function wikidataIT(id) {
  ...
  try {
    ...
    const r = await fetch('https://query.wikidata.org/sparql?format=json&query=' + encodeURIComponent(q), {
      headers: { accept: 'application/sparql-results+json', 'user-agent': 'ItalyReleaseChecker/1.6' }
    });
    if (!r.ok) return false;
...
// --- META: verdetto sul singolo film (logica multi-fonte invariata) ---
app.get('/meta/movie/:id.json', async (req, res) => {
  try {
    const id = req.params.id.replace(/^tmdb:/, '');
    if (!/^\d+$/.test(id)) return res.status(400).json({ error: 'Use a TMDB movie id' });
    const m = await tmdb('/movie/' + id);
    if (!m) return res.status(404).json({ error: 'Movie not found' });
    const [a, b, c, d] = await Promise.allSettled([
      tmdbIT(id), imdbIT(m.imdb_id), wikidataIT(id), justwatch(m.title || m.original_title)
    ]);
    ...
    res.json({
      meta: {
        ...
        description: released ? '🇮🇹 USCITO IN ITALIA' : '🚫 NON USCITO IN ITALIA'
      }
    });
  } catch (e) {
    res.status(500).json({ error: 'Release check failed' });
  }
});

// --- CATALOG: ora popolato davvero, con ricerca TMDB. Senza "search" mostra i popolari. ---
...
app.listen(PORT, () => console.log('Italy Release Checker 1.6 listening on ' + PORT));
